...
// ── Playoff Sheet CSV ────────────────────────────────────────────────────────
export interface PlayoffRawTeam {
  name: string;
  seed: number;
  players: { name: string; handicap: number }[];
}

export function parsePlayoffCSV(csv: string): PlayoffRawTeam[] {
  const lines = csv.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
  const teams: PlayoffRawTeam[] = [];

  // Parse standings section (rows 1-N until empty) to get seeding
  ...
  for (; i < lines.length; i++) {
    ...
    if (!team || isNaN(place)) break;
    seedMap[team] = place;
  }

  // Find HANDICAPS section
  let handicapStart = -1;
  for (let j = i; j < lines.length; j++) {
    if (/handicaps/i.test(lines[j])) {
      handicapStart = j;
      break;
    }
  }
  if (handicapStart === -1) return [];

  // Parse handicap blocks
  // Structure: team names row, then player rows (name, handicap pairs at columns 2-3, 5-6, 8-9)
  // Teams are at cols 2, 5, 8 in the header row
  // Players are at cols 2-3, 5-6, 8-9

  let row = handicapStart + 1;
  while (row < lines.length) {
    const cols = parseCSVLine(lines[row]);
    // Look for team name row (non-empty at col 2, 5, or 8)
    const teamPositions = [2, 5, 8];
    const foundTeams: { name: string; col: number }[] = [];

    for (const col of teamPositions) {
      const val = (cols[col] || '').trim();
      if (val && seedMap[val] !== undefined) {
        foundTeams.push({ name: val, col });
      }
    }
    ...
    const teamEntries: Record<string, { name: string; handicap: number }[]> = {};
    for (const t of foundTeams) {
      teamEntries[t.name] = [];
    }

    // Read player rows below
    row++;
    while (row < lines.length) {
      const pCols = parseCSVLine(lines[row]);
      let foundAny = false;

      for (const t of foundTeams) {
        const nameCol = t.col;
        const hcpCol = t.col + 1;
        const playerName = (pCols[nameCol] || '').trim();
        const hcpVal = parseInt((pCols[hcpCol] || '').trim());

        if (playerName && !isNaN(hcpVal)) {
          teamEntries[t.name].push({ name: playerName, handicap: hcpVal });
          foundAny = true;
        }
      }

      if (!foundAny) {
        // Could be a blank separator row between regular and sub players
        // Check next row to see if more players follow
        if (row + 1 < lines.length) {
          const nextCols = parseCSVLine(lines[row + 1]);
          let hasNext = false;
          for (const t of foundTeams) {
            const nm = (nextCols[t.col] || '').trim();
            const hc = parseInt((nextCols[t.col + 1] || '').trim());
            if (nm && !isNaN(hc)) { hasNext = true; break; }
          }
          if (hasNext) { row++; continue; }
        }
        break;
      }
      row++;
    ...
    // Add teams
    for (const t of foundTeams) {
      if (teamEntries[t.name].length > 0) {
        teams.push({
          name: t.name,
          seed: seedMap[t.name] || 99,
          players: teamEntries[t.name],
        });
      }
    }
  }
