...
export default function HcpCalcCard({
  selectedPlayersA,
  selectedPlayersB,
  teamAName,
  teamBName,
...
function TeamBlock({ players, teamName, accentColor, label }: {
    ...
    <div style={{ marginBottom: '0.9em' }}>
      ...
      {players.map(p => (
        <div key={p.name} style={{
          ...
          padding: '0.4em 0.6em', borderRadius: '0.3em',
...
function EdgeDisplay({ projA, projB, teamAName, teamBName }: {
  projA: number; projB: number; teamAName: string; teamBName: string;
}) {
